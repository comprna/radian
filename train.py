...
def train(shards_dir, checkpoint, epoch_to_resume, config_file):
    ...
    train_files = glob("/g/data/xc17/Eyras/alex/rna-basecaller/shards/debugging/mixed-labels-10/train/*.tfrecords")
    train_dataset = get_dataset(train_files, config, val=False)
    train_dataset_for_eval = get_dataset(train_files, config, val=True)

    val_files = glob("/g/data/xc17/Eyras/alex/rna-basecaller/shards/debugging/mixed-labels-10/val/*.tfrecords")
